// Final seeding script for ReportTypes - fixed version
...
async function seedReportTypes() {
  ...
  try {
    ...
    const existing = db.exec("SELECT COUNT(*) as count FROM ReportTypes");
    const existingCount = existing.length > 0 ? existing[0].values[0][0] : 0;

    if (existingCount > 0) {
      console.log(`✅ ReportTypes already has ${existingCount} entries`);
    ...
    for (const [TypeCode, TypeName, Category, Description, DisplayOrder] of reportTypes) {
      try {
        db.run('INSERT INTO ReportTypes (TypeCode, TypeName, Category, Description, DisplayOrder, IsActive) VALUES (?, ?, ?, ?, ?, 1)',
